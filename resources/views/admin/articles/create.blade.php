...
<div class="max-w-4xl mx-auto">
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('test.store.article') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <div class="mb-6">
                ...
                <textarea id="excerpt" 
                          ...
                          rows="3"
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                          placeholder="Résumé de l'article (optionnel, sera généré automatiquement si vide)">{{ old('excerpt') }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
